...
- Um **Projeto** tem **Fases**, e cada Fase tem **Tarefas** (type: task/milestone/meeting), cada uma com um **Executor** (obrigatório) e opcionalmente um **Validador**. Tarefas do tipo task têm plannedStart/plannedEnd/durationDays; marcos e reuniões têm plannedDate (reunião também tem durationHours); qualquer uma pode ter dependsOn (outras tarefas das quais depende). Use \`list_phases\` pra descobrir os ids das fases de um projeto antes de criar/mover uma tarefa, e \`update_task\` pra editar uma tarefa já existente (inclui mover pra outra fase via \`newPhaseId\`).
